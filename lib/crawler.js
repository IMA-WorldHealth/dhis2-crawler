...
class Crawler extends EventEmitter {
  ...
  async goToDashboardById(ident, _delay = 10) {
  ...
  async gatherTablesFromDashboard() {
    ...
    if (saveScreenshots) {
      await this.page.screenshot({ path: '/tmp/screenshot-before.png', fullPage : false });
    }
    ...
    if (saveScreenshots) {
      await this.page.screenshot({ path: '/tmp/screenshot-after.png', fullPage : false });
    }
    ...
    if (saveScreenshots) {
      await this.page.screenshot({ path: '/tmp/screenshot-all.png', fullPage : true });
    }
