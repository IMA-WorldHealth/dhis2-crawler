...
const DHIS2_229_SELECTOR = '.d2-ui-control-bar-contents span:contains("%SELECTOR%)';
const DHIS2_232_SELECTOR = 'div a[href] span:contains("%SELECTOR%")';
...
class Crawler extends EventEmitter {
  ...
  async generateIdForTextNode(text) {
    ...
    // NOTE - older DHIS2 versions use this.
    await this.page.evaluate(
      (txt, uid) => {
        const selector = DHIS2_232_SELECTOR.replace('%SELECTOR%', txt);
        window.$(selector).prop('id', uid);
      },
      text,
      ident,
    );
