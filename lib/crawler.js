...
class Crawler extends EventEmitter {
  ...
  async gatherTablesFromDashboard() {
    ...
    // grab tables
    const tables = await this.page.$$('.pivot-table-container > table');

    // convert tables to URIs
    const uris = await Promise.all(tables.map(table => table.screenshot({ encoding: 'base64', omitBackground : true })));

    const labels = await this.page.$$eval('.pivot-table-container > table', tables => tables.map(table =>
      table
        .parentElement
        .parentElement
        .parentElement
    ...
    const datatables = uris.map((uri, idx) => ({ uri : `data:image/png;base64,${uri}`, label : labels[idx] }));

    this.emit('event', `Pulled ${datatables.length} tables from the dashboard.`);
